export const SHOW_LOADER = 'APP/SHOW_LOADER';
export const HIDE_LOADER = 'APP/HIDE_LOADER';
export const ERROR = 'APP/ERROR';
export const CLEAR_ERROR = 'APP/CLEAR_ERROR';
export const MESSAGE = 'APP/MESSAGE';
export const CLEAR_MESSAGE = 'APP/CLEAR_MESSAGE';
export const SIDEBAR_TOGGLE = 'APP/SIDEBAR_TOGGLE';

// AUTH
export const CHANGE_AUTH_FORM = 'AUTH/CHANGE_AUTH_FORM';

// COMPANY
export const GET_COMPANIES = 'COMPANY/GET_ALL';
export const CLEAR_COMPANIES = 'COMPANY/CLEAR_ALL';
export const SELECT_COMPANY = 'COMPANY/SELECT';

// CHARTS
export const GET_DAILY_CHART = 'CHART/GET_DAILY_CHART';
export const GET_MONTHLY_CHART = 'CHART/GET_MONTHLY_CHART';
export const GET_FINANCE_CHART = 'CHART/GET_FINANCE_CHART';

// WORK_SHIFTS
export const GET_WORK_SHIFTS = 'SHIFTS/GET_WORK_SHIFTS';
export const SELECT_WORK_SHIFT = 'SHIFT/SELECT';

// ACCOUNTS
export const GET_ACCOUNTS = 'ACCOUNTS/GET';

// EXPENSES OUTSIDE
export const GET_EXP_OUTSIDE = 'EXP_OUTSIDE/GET';
