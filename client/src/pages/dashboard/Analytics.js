...
export const Analytics = () => {
  ...
  const monthlyChartData = useSelector(
    (state) => state.charts.monthlyChartData
  );
  const financeChartData = useSelector(
    (state) => state.charts.financeChartData
  );
  const [nowDayData, setNowDayData] = useState({
    revenue: 0,
    profit: 0,
    receipts: 0,
  });
      ...
      <div className="row mt-4">
        <div className="col-md-8 col-lg-9 col-sm-12">
          <div className="card">
            ...
            <Line
              data={createDailyChart(dailyChartData)}
        ...
        <div className="col-md-4 col-lg-3 col-sm-12">
          <div className="card pb-2">
            ...
            <div style={{ height: '17.5rem' }}>
              <Pie
                data={createFinanceChart(financeChartData)}
                options={{ maintainAspectRatio: false }}
              />
            </div>
            <div
              className="d-flex justify-content-start flex-column"
              style={{ padding: '17px', fontSize: '20px' }}
            >
              <span
                style={{
                  color: '#2ac02a',
      ...
      <div className="row mt-4">
        ...
        <div className="col-md-4 col-lg-3 col-sm-6">
          <select className="form-control" onChange={changeYearHandler}>
            {years.map((y) => (
              <option value={y} selected={y === selectedYear} key={y}>
                {y}
              </option>
            ))}
          </select>
      ...
      <div className="row mt-4">
        ...
        <div className="col-lg-3 col-md-12 col-sm-12">
          <div className="card" style={employeeCard}>
            ...
            <MDBListGroup>
              <MDBListGroupItem>
                <i className="fas fa-user-tie mr-2" style={{ color: 'gold' }} />
                Маша <i className="fas fa-arrow-alt-circle-right" /> 1300 продаж
              </MDBListGroupItem>
              <MDBListGroupItem>
                <i
