...
const { generateDataByShifts, getDailyChartData, getMonthlyChartData } = require('./functions');
...
router.post('/now', auth, (req, res) => {
    ...
    .then(async workShifts => {
        ...
        res.json({revenue: revenueSum, profit: revenueSum - costAmount, receipts});
    })
    .catch(error => {
        res.status(400).json({message: config.get('unknownErrorMessage'), error});
    });
});
